...
export class FixSuggesterService {
  ...
  private async callModel(
        ...
        messages: [
          {
            ...
            content:
              ...
              "1 to 8 changes, each independently actionable by a person editing the workflow in the n8n editor. If the cause is external (expired credential, third-party outage, rate limit), say so in the summary and make the changes defensive (retry settings, error handling).",
          },
    ...
    const suggestion = SuggestionShape.safeParse(parsed);
    ...
    return {
      ...
      changes: suggestion.data.changes,
      mechanism:
